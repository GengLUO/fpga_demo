...
int main(int argc, char** argv) {
  ...
  try {
    ...
    if (!input_path.empty()) {
      ...
      if (!load_bin(input_path, in_ptr, bytes_inout)) return 2;
    } else {
    ...
    if (!ref_out_path.empty()) {
      ...
      if (!load_bin(ref_out_path, ref.data(), bytes_inout)) {
        ...
        compare_and_report(out_ptr, ref.data(), static_cast<size_t>(H) * W * C,
                          H, W, C, "FINAL");
        // simple pass/fail check with common FP32 thresholds
